...
router.post("/order-detail", async (req, res) => {
    let id = req.body.id;       
    let result = await con.queryAsync("SELECT product.title, t1.price, t1.amount FROM (SELECT * FROM order_detail WHERE order_id=?) AS t1 INNER JOIN product ON t1.product_id=product.id", [id]);
    console.log(result);
    res.send(result)
})
